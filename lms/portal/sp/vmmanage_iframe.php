...
if (! api_is_admin ()) api_not_allowed ();
  $g_dhcp=  getgpc("dhcp_page_nr");
  $g_keyword=  getgpc('keyword');
  $g_vmaddres= getgpc('vmaddres');
if(!isset($g_dhcp) && !isset($g_keyword) && !isset($g_vmaddres)) {
...
function get_sqlwhere() {
    ...
    if (is_not_blank ( $g_keyword )) {
        ...
        $sql_where .= " AND (id LIKE '%" . trim ( $keyword ) . "%' OR addres LIKE '%" . trim ( $keyword ) . "%' OR nicnum LIKE '%" . trim ( $keyword ) . "%' OR system LIKE '%" . trim ( $keyword ) . "%')";
    }
    $user=  getgpc('user');
    if (is_not_blank ( $user)) {
        $sql_where .= " AND user_id=" .$user;
    }
    $course=getgpc('course');
...
?>
<aside id="sidebar" class="column remote open">
...
<section id="main" class="column">
    <h4 class="page-mark">当前位置：<a href="<?=URL_APPEDND;?>/main/admin/index.php">平台首页</a> &gt;远程协助</h4>
    <div  class="managerSearch">
        <?php  $form->display ();?>
        <span width="90%" class="searchtxt right"></span>
    </div>
    <article class="module width_full hidden ip">
        <h3 class="vmip"><img src="../../themes/default/images/base1.gif" align="absmiddle" >  节点地址</h3>
        <div class="vmContent">
