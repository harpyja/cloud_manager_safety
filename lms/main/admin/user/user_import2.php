...
//获取user表用户数量
function user_num() {
        $sql = "SELECT COUNT(user_id) AS total_number_of_items FROM user WHERE  active=1 and user_id != 1 " ;
...
function parse_upload_data($file) {
	...
	if (is_array ( $users ) && count ( $users ) > 0) {
		foreach ( $users as $index => $user ) {
			$user ['姓名'] = _convert_charset ( $user ['姓名'], SYSTEM_CHARSET, $import_encoding );
			$user ['工号，学号，编号'] = _convert_charset ( $user ['工号，学号，编号'], SYSTEM_CHARSET, $import_encoding );
...
function validate_data($users) {
	...
	$emails = array ();
	foreach ( $users as $index => $user ) {
		$tmp_cur_user = $user ['登录名'];
		if (! empty ( $tmp_cur_user ) and ! is_root ( $tmp_cur_user )) {
			$user ['Status'] = STUDENT;
			
			//1. 检查必填字段
			$mandatory_fields = array ('登录名' ); //某些不能为空的字段
...
function complete_missing_data($users, $import_dept = 0) {
	foreach ( $users as $index => $user ) {
		...
		if (! isset ( $login_name ) || strlen ( $login_name ) == 0) {
			...
			$users [$index] ['登录名'] = $username;
		}
		//2. generate a password if necessary
		if (empty ( $user ['密码'] ) || strlen ( $user ['密码'] ) == 0) {
			$users [$index] ['密码'] = api_generate_password ();
		}
		...
		if (empty ( $import_dept )) { //使用文件中部门
			if (is_not_blank ( $user ['部门'] )) {
				$tbl_dept = Database::get_main_table ( TABLE_MAIN_DEPT );
				$sql = "SELECT id FROM " . $tbl_dept . " WHERE dept_no=" . Database::escape ( trim ( $user ['部门'] ) );
				$users [$index] ['部门'] = Database::get_scalar_value ( $sql );
			} else {
				$users [$index] ['部门'] = 0;
			}
		} else { // 导入到某部门中去
			$users [$index] ['部门'] = $import_dept;
		}
	}
	return $users;
}
...
function save_data($users) {
	...
	foreach ( $users as $index => $user ) {
		...
		if (UserManager::is_username_available ( $username )) { //不存在时
			$user_id = UserManager::create_user ( $user ['姓名'], $user ['职务'], $user ['Status'], $user ['邮箱'], $username, $user ['密码'], $user ['工号，学号，编号'], $platformLanguage, $user ['手机'], $user ['Picture'], $user ['AuthSource'], '0000-00-00 00:00:00', 
					$user ['是否启用'], $user ['DeptID'], '', $user ['性别'], 1, $user ['IDCard'], $user ['ZipCode'], $user ['Address'], $user ['手机'], $user ['QQ'], $user ['MSN'], '', $restrict_org_id, $ip );
...
//					'phone' => $user ['Phone'],
		...
		if ($sendMail) {
			...
			$emailTo = trim ( $user ['邮箱'] );
			$emailSubject = get_lang ( 'YourReg' );
			$emailBody = get_lang ( 'Dear' ) . ' ' . $user ['姓名'] . ": <br/>" . get_lang ( 'YouAreReg' ) . ' "' . get_setting ( 'siteName' ) . '". <br/>' . get_lang ( 'Settings' ) . "<br/>";
			$emailBody .= get_lang ( 'TheU' ) . ' : ' . $user ['登录名'] . "<br/>" . get_lang ( 'siteAddr' ) . ' : ' . api_get_path ( WEB_PATH ) . " <br/><br/>";
