...
$resultcategories = array ('客户端设置', '服务端设置' ,"数据同步服务器");
...
/**Data syncchronization；changzf 2012/11/24 13:35 end*/


$ini_ite = get_file('/etc/cloudschedule/enable');
if($_GET['category'] == '服务端设置'){
...
}elseif($_GET['category'] == '数据通道设置')
{

    $dir = URL_ROOT."/www".URL_APPEDND."/main/admin/vmmanage/";
    $ssh = ireaddir($dir);
//    var_dump($ssh);
    foreach ($ssh as $k => $v){
    ...
    //root/.ssh/
}elseif(htmlspecialchars($_GET['category']) == '数据同步服务器'){
    $form->addElement ( 'checkbox', 'open_synchronization','开启数据同步服务器&nbsp;','', array ('id' => 'open_synchronization' ) );
}
//elseif(htmlspecialchars($_GET['category']) == '数据同步'){
...
//if(htmlspecialchars($_GET['category']) == '数据同步'){
...
if ($form->validate ()) { //处理保存
   ...
   $str = '';
    ...
    /**open data syncchronization ; changzf 2012/11/24 14:00 start*/
    if(htmlspecialchars($_GET['category']) == '数据同步服务器'){
        if($values["open_synchronization"]=='1'){
            ...
            $result = sizeof($vmsummarys);//addres count
            ...
            if($result!=''){
                for($i=0;$i<$result;$i++){
                    $j=$i+1;
                    $address=explode(";",$vmsummarys[$i][0]);
                    $addres_count=sizeof($address);
                    if($addres_count=='2'){
                        $add1=$address[0];
                        $add2=$address[1];
                        $table.='
		            ...
		            <td style="width:100px;text-align:center"">'.$add1.'</td>
		            <td style="width:100px;text-align:center"">
		                <a href=centralized.php?category=数据同步&actions='.$add2.'>数据同步</a>
		            </td></tr>';
                    }
                }
            }else{
    ...
    /**open data syncchronization ; changzf 2012/11/24 14:00 end*/
...
//var_dump($ini_ite);
   ...
   // exec("sudo -u root /sbin/cloudvmweb.sh");
    //api_redirect ( 'centralized.php' );
}
...
<section id="main" class="column">
    <h4 class="page-mark">当前位置：<a href="<?=URL_APPEDND;?>/main/admin/penetration/index.php">平台首页</a> &gt; 集中管理设置</h4>
