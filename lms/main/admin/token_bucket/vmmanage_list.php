...
function get_sqlwhere() {
    ...
    if (is_not_blank ( $_GET ['keyword'] )) {
        $keyword = Database::escape_string ( getgpc('keyword'), TRUE );
...
function get_vm_data($from, $number_of_items, $column, $direction) {
   ...
   // var_dump($vmaddres);
    if($vmaddres){
        $select = "select id as co9,addres as co10 , nicnum as co11, system as co12 ,user_id as co13 ,lesson_id as co14,vmid as co15,port as co16 ,group_id as co17 FROM vmtotal where addres = '$vmaddres'";
    }else{
        $select = "select id as co9,addres as co10 , nicnum as co11, system as co12 ,user_id as co13 ,lesson_id as co14,vmid as co15,port as co16 ,group_id as co17 FROM vmtotal ";
    }
