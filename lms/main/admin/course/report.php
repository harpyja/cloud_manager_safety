...
function content_filter($id){
    ...
    $result.="<a href='report.php?action=view&id=".intval($id)."'><span style='text-align:left'>".$comment."</span></a>";
    return $result;
}
...
function get_report_data($from, $number_of_items, $column, $direction) {
    $report = Database::get_main_table ( report );
    $sql = "select `id`,`report_name`,`user`,`id`,`description`,`score`,`comment`,`id` ,`id` FROM  ".$report." where `status`=1 ";
